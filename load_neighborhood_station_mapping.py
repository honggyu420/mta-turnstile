...
def drop_tables(cur, conn):
    drop_query = "drop table if exists station_neighborhood_mapping"
...
def create_tables(cur, conn):
    create_query = """
    create table if not exists station_neighborhood_mapping(
...
def populate_tables(cur, conn):
    raw_data_path = 'data/station_neighborhood_mapping.csv'
    ...
    cur.copy_expert("COPY station_neighborhood_mapping FROM STDIN DELIMITER ',' CSV HEADER;", f)
    conn.commit()
